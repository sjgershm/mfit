...
param(1).link = @(x) log(x); % log link function that transforms x from [0, +inf) to (-inf, +inf)
...
param(2).link = @(x) log(x) - log(1-x); % logit link function transforms x from [0,1] to (-inf, +inf)
param(2).lb = 0;
param(2).ub = 1;
...
results(1) = mfit_optimize_hierarchical(@rllik,param(1:2),data,nstarts);
disp('... Fitting model 2');
results(2) = mfit_optimize_hierarchical(@rllik2,param,data,nstarts);
